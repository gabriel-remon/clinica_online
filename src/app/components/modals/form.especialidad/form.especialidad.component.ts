...
export class FormEspecialidadComponent {
  ...
  readonly dialogRef = inject(MatDialogRef<FormEspecialidadComponent>);

  tipoForm = "Agregar obra social"
  obraSocialSvc = inject(EspecialidadService)
  imagen: any;
  ...
  submit() {
    ...
    this.obraSocialSvc.newData(nuevoActor as Especialidad, this.imagen).then((data) => {
      if (data.estado) {
