...
import { Firestore, query, QueryDocumentSnapshot, QuerySnapshot, addDoc, collection, getFirestore, onSnapshot, orderBy, deleteDoc, setDoc, where } from '@angular/fire/firestore';
...
export class EspecialidadService {
  ...
  getEspecialidadId(id_especialidad:string ,funcion: (repartidores: Especialidad) => void, finaly?: () => void) {
    // Crear una consulta ordenada por el campo 'fecha' en orden ascendente
    const mensajeRef = collection(this.dbFirebase, this.tabla)
    const q = query(mensajeRef,where("id","==",id_especialidad))
